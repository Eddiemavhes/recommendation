...
class PDFTextExtractor:
    def extract_text(self, file_obj):
        ...
        print(f"PDFTextExtractor: Processing file_obj type {type(file_obj)}")
        
        if isinstance(file_obj, (bytes, bytearray)):
            print("Processing bytes/bytearray input")
            pdf_file = io.BytesIO(file_obj)
        else:
            pdf_file = file_obj
        ...
        if not extracted_text.strip():
            try:
                ...
                if extracted_text.strip():
                    print(f"Extracted {len(extracted_text)} chars with pdfplumber")
                    return extracted_text.strip()
            except Exception as e:
